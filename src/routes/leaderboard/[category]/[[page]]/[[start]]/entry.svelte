...
>
	<div class="flex gap-4 justify-start align-middle items-center mx-2">
		<div class="text-green-800 dark:text-green-300">
			<h1>
				<span class="text-sm xs:text-md sm:text-2xl">#</span><span class="text-lg xs:text-xl sm:text-3xl">{rank}</span>
			</h1>
		</div>
		<div class="flex flex-col">
			<h1 class="inline text-sm xs:text-xl sm:text-2xl font-semibold text-start">{ign}</h1>
			<h4 class="text-xs xs:text-sm sm:text-md text-start overflow-hidden text-ellipsis whitespace-nowrap">{cute_name}</h4>
		</div>
	</div>
