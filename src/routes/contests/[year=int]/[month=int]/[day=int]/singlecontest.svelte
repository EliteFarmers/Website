<script lang="ts">
	...
	$: sorted = entries?.sort((a, b) => (b?.collected ?? 0) - (a?.collected ?? 0)).slice(0, expand ? undefined : 10) ?? [];
</script>
...
<div class="flex-1 basis-1/4 justify-between h-full items-center rounded-md shadow-md bg-gray-100 dark:bg-zinc-800">
	<div class="flex flex-col items-center justify-start p-4 space-y-2">
		...
		<h4 class="text-lg">
			{entries?.length ?? 0} / {participants !== -1
				? participants
	...
	<div class="flex flex-col md:flex-row gap-4 justify-center m-2 items-center">
		...
		{#if entries.length > 10}
			<button class="whitespace-nowrap bg-gray-200 dark:bg-zinc-700 hover:bg-gray-300 dark:hover:bg-zinc-900 p-1 px-2 rounded-md" on:click={() => expand = !expand}>{expand ? 'Collapse' : 'Show All'}</button>
		{/if}
	</div>
</div>
