...
<div class="flex flex-col md:flex-row flex-1 gap-2 w-full max-w-[116rem] rounded-md justify-between items-center p-4 {current ? 'bg-yellow-100 dark:bg-yellow-900' : 'bg-gray-100 dark:bg-zinc-800'}">
	<div class="flex flex-col gap-2 items-center md:items-start">
		...
		<!-- Time -->
		<h4 class="text-xl whitespace-nowrap font-mono font-semibold">{new Date(timestamp * 1000).toLocaleString(undefined, {
			timeStyle: 'short',
			dateStyle: 'medium',
		})}</h4>
