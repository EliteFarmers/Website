...
{:else}
	{#await data.profileData}
		<div
			...
			in:fade={{ delay: 300 }}
		>
			<LoaderCircle class="size-32 animate-spin text-muted-foreground" />
...
{#snippet profileError(profile: { code: number; error: string })}
	...
	<div class="item m-12 mb-32 flex flex-row justify-center gap-4 text-center">
		<a href={resolve('/')} class="shadow-gray 0 rounded-md bg-muted p-4 shadow-md hover:bg-card">Return Home</a>
		<button onclick={() => location.reload()} class="shadow-gray rounded-md bg-muted p-4 shadow-md hover:bg-card"
			>Reload</button
		>
	</div>
{/snippet}
