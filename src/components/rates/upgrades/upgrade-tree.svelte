...
>
	<div class="group relative flex items-center gap-3 p-2 transition-colors hover:bg-white/5">
		{#if isExpandable}
			<Collapsible.Trigger
		...
		<div class="w-full min-w-0 flex-1 py-1">
			<div class="grid w-full grid-cols-3 items-center gap-4 pr-4">
					...
					{#if upgrade.api === false}
						<Popover.Mobile>
							...
							<p class="max-w-sm text-sm">
								This fortune source is not available in the Hypixel API. Configure settings on this page
				...
				<UpgradeCostPer {upgrade} totalCost={cost} />
			</div>
		</div>
		...
		<div class="flex shrink-0 items-center gap-1">
			{#if upgrade.wiki}
				<a
					...
					class="text-muted-foreground hover:text-primary p-1 transition-colors"
				>
					<Info size={14} />
				</a>
			{/if}
	...
	{#if isExpandable}
		<Collapsible.Content>
			<div class="flex flex-col border-t">
				{#if hasDetails}
					<div class="flex w-full flex-row items-start justify-between gap-4 p-2">
