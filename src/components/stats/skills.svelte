<script lang="ts">
	...
	const ranks = $derived(ctx.ranks?.skills);
			...
			{#snippet child({ props })}
				<div class="flex w-full flex-row items-end justify-center gap-4">
					<div class="flex w-full flex-1 flex-col items-end justify-center gap-4 md:flex-row">
						...
						<Skillbar name="Garden" rank={ctx.ranks?.profile?.garden} progress={getGardenLevel(gardenXp)} />
					</div>
					<div class="md:hidden">
