...
>
	...
	<div class="flex flex-row gap-4 z-10 items-center">
		...
		<div class="flex flex-col gap-1 items-start">
			<h2 class="text-lg sm:text-2xl md:text-3xl font-semibold text-white">{event.name}</h2>
			<EventType type={event.type ?? 1} />
		</div>
	</div>
	<div class="flex flex-row gap-2 font-semibold items-center z-10 sm:text-sm md:text-lg text-white">
