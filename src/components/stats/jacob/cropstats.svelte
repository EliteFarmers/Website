...
<div class="flex flex-wrap gap-4 items-center justify-center max-w-5xl">
	{#each highest as [ crop, amount ] (crop)}
		...
		<div class="flex-1 basis-48 flex flex-row justify-between bg-gray-100 dark:bg-zinc-800 rounded-md p-2 items-center">
			...
			{#if unique} 
				<img src="/images/medals/{unique}.webp" alt="{unique} Medal" class="w-10 h-10 p-1 pixelated highest-bracket">
			{/if}
		</div>
	{/each}
