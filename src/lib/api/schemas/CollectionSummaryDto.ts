/**
 * Generated by orval 🍺
 * Do not edit manually.
 * Elite API
 * A backend API for https://eliteskyblock.com/ that provides Hypixel Skyblock data.
 * <br><br>
 * Use of this API requires following the [Elite API TOS](https://eliteskyblock.com/apiterms). This API is not affiliated with Hypixel or Mojang.
 * OpenAPI spec version: v1
 */
import type { DictionaryOfStringAndBossCollectionProgressDto } from './DictionaryOfStringAndBossCollectionProgressDto';
import type { KuudraTierProgressDto } from './KuudraTierProgressDto';

/**
 * Derived summary of boss/Kuudra collection tier progression computed from
 *             Collections and the staged collection metadata.
 */
export interface CollectionSummaryDto {
	bosses: DictionaryOfStringAndBossCollectionProgressDto;
	kuudra?: KuudraTierProgressDto | null;
}
