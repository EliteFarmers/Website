/**
 * Generated by orval v7.16.0 🍺
 * Do not edit manually.
 * Elite API
 * A backend API for https://eliteskyblock.com/ that provides Hypixel Skyblock data.
<br><br>
Use of this API requires following the [Elite API TOS](https://eliteskyblock.com/apiterms). This API is not affiliated with Hypixel or Mojang.
 * OpenAPI spec version: admin-v1
 */
import type { GuideAssetType } from './GuideAssetType';
import type { GuideAssetDtoImage } from './GuideAssetDtoImage';
import type { GuideAssetDtoLitematic } from './GuideAssetDtoLitematic';

export interface GuideAssetDto {
	id: string;
	type: GuideAssetType;
	fileName: string;
	contentType: string;
	sizeBytes: bigint;
	createdAt: string;
	/** @nullable */
	image?: GuideAssetDtoImage;
	/** @nullable */
	litematic?: GuideAssetDtoLitematic;
}
