...
import {
	FARMING_ACCESSORIES_INFO,
	FarmingAccessoryInfo,
} from '../../items/accessories.js';
...
export const GENERAL_FORTUNE_SOURCES: DynamicFortuneSource<FarmingPlayer>[] = [
	{
		...
		current: (player) => {
			return (
				(player.options.farmingLevel ?? 0) *
				FARMING_LEVEL.fortunePerLevel
			);
		},
	},
	{
		...
		exists: () => true,
		max: () =>
			PEST_BESTIARY_SOURCE.maxLevel *
			PEST_BESTIARY_SOURCE.fortunePerLevel,
		current: (player) => {
	...
	},
	{
		...
		exists: () => true,
		max: () =>
			ANITA_FORTUNE_UPGRADE.maxLevel *
			ANITA_FORTUNE_UPGRADE.fortunePerLevel,
		current: (player) => {
			return (
				(player.options.anitaBonus ?? 0) *
				ANITA_FORTUNE_UPGRADE.fortunePerLevel
			);
		},
	},
	{
		...
		current: (player) => {
			return (
				(player.options.plotsUnlocked ?? 0) *
				UNLOCKED_PLOTS.fortunePerLevel
			);
		},
	},
	{
		...
		exists: () => true,
		max: () =>
			COMMUNITY_CENTER_UPGRADE.maxLevel *
			COMMUNITY_CENTER_UPGRADE.fortunePerLevel,
		current: (player) => {
			return (
				(player.options.communityCenter ?? 0) *
				COMMUNITY_CENTER_UPGRADE.fortunePerLevel
			);
		},
	},
	{
		...
		wiki: (player) => {
			const highest = player.activeAccessories.find(
				(a) =>
					a.info.family ===
					FARMING_ACCESSORIES_INFO.FERMENTO_ARTIFACT?.family
			);
			return (
				highest?.info.wiki ??
				FARMING_ACCESSORIES_INFO.CROPIE_TALISMAN?.wiki
			);
		},
		max: () =>
			FARMING_ACCESSORIES_INFO.FERMENTO_ARTIFACT?.baseStats?.[
				Stat.FarmingFortune
			] ?? 0,
		current: (player) => {
			const highest = player.activeAccessories.find(
		...
		info: (player) => {
			...
			const first = !highest
				? FARMING_ACCESSORIES_INFO.CROPIE_TALISMAN
				: undefined;
			return {
				item: highest?.item,
				info: highest?.info,
				...
				maxInfo:
					highest?.getLastItemUpgrade()?.info ??
					FARMING_ACCESSORIES_INFO.FERMENTO_ARTIFACT,
			};
		},
	},
	{
		...
		exists: () => true,
		max: () =>
			REFINED_TRUFFLE_SOURCE.maxLevel *
			REFINED_TRUFFLE_SOURCE.fortunePerLevel,
		current: (player) => {
			return (
				(player.options.refinedTruffles ?? 0) *
				REFINED_TRUFFLE_SOURCE.fortunePerLevel
			);
		},
	},
	{
		...
		max: () => {
			const accessory = FarmingAccessory.fakeItem(
				FARMING_ACCESSORIES_INFO.POWER_RELIC as FarmingAccessoryInfo
			);
			return (
				accessory
					?.getProgress()
					?.reduce((acc, p) => acc + p.maxFortune, 0) ?? 0
			);
		},
		current: (player) => {
			const accessory = player.accessories.find(
				(a) => a.info.skyblockId === 'POWER_RELIC'
			);
		...
		info: (player) => {
			...
			return {
				...
				nextInfo: fake
					? fake.info
					: accessory?.getNextItemUpgrade()?.info,
